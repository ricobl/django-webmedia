...
@register.filter
def remove_tag_blocks(value, taglist):
    ...
    tags = '|'.join(taglist.split())
    
    # Regex para remover tags com conteúdo <tag></tag>
    re_full_tags = re.compile('<(%s)[^>]*?>.*?</\\1>' % tags, re.S)
    # Regex para remover tags simples <tag />
    re_auto_tags = re.compile('<(%s)[^>]*?/?>' % tags, re.S)
    
    value = re_full_tags.sub('', value)
    value = re_auto_tags.sub('', value)
...
def query_string(context, **kwargs):
    """
    Extrai a query-string do contexto e atualiza as variáveis
    para formar um novo string.
    """
    req_get = context.get('request').GET.copy()
    # Usando setitem em cada chave
    # um update geral pode duplicar itens
    for k in kwargs:
        req_get.__setitem__(k, kwargs[k])
    return req_get.urlencode()

# Expressão regular para encontrar blocos de comentários condicionais
RE_CONDITIONAL_COMMENTS = re.compile(r'<!--\[if [^]]*?\]>.*?<!\[endif\]-->', re.S)

@register.filter
def remove_cc(text):
    """
    Remove comentários condicionais de um texto.
    """
    return RE_CONDITIONAL_COMMENTS.sub('', text)
